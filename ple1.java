...
public class SampleClass {
    ...
    public boolean getDownloadedFilePath(IArgument downloadPath) {
        ...
        try {
            ...
            String fileName = this.driver.executeScript(
    ...
    public boolean questDatePicker(String numberString, String format, IArgument store) {
        ...
        try {
            ...
            String year = date1[2];
            this.driver.findElement(FindBy.xpath("//tr[@role='row']/td/div[normalize-space(text())='" + year + "']"))
                    .click();
            Thread.sleep(2000L);
        ...
        try {
            ...
            String sh = this.driver.executeScript(
                    "function getElementByXpath(path) {return document.evaluate(path, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;}var a = getElementByXpath(\""
                            + xpath + "\");return a.value;",
                    new Object[0]).toString();
            this.driver.getExecutionLogReporter().reportSubStep(Status.PASSED, "executeScript", List.of(xpath));
            SubStep sa = SubStep.builder().status(Status.PASSED).description("xpath").testDataValues(List.of(sh, sh))
                    .build();
    ...
    public boolean verifyAscendingSort() throws InterruptedException {
        ...
        try {
            ...
            for (int i = 1; i <= lenCount; i++) {
                ...
                String sh = this.driver.executeScript(
                        "function getElementByXpath(path) {return document.evaluate(path, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;}var a = getElementByXpath(\""
                                + s1 + "\");return a.innerText;",
                        new Object[0]).toString().trim().toLowerCase();
                strings.add(sh);
            }
            String en = this.driver.executeScript(
                    "function getElementByXpath(path) {return document.evaluate(path, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;}var a = getElementByXpath(\""
                            + nextPage + "\");return a.disabled;",
    ...
    public boolean verifyNoDuplicatesInCheckboxDropdown() {
        ...
        try {
            ...
            LinkedHashSet<String> hashSet = new LinkedHashSet<>();
            for (int i = 1; i <= lenCount; i++) {
                String xpath2 = "(//li[@class='p-ripple p-element p-multiselect-item']//span[@class='ng-star-inserted'])["
                        + i + "]";
                String a = this.driver.findElement(FindBy.xpath(xpath2)).getText();
                hashSet.add(a);
            }
